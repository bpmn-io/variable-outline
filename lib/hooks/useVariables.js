...
export const getVariables = async ({ variableResolver, selection, filter }) => {
  ...
  const availableVariables = filteredVariables
    ...
    .filter(typeFilter(filter.filterType))
    .filter(writtenOnlyFilter(filter.writtenOnly, filter.selectedElements));
...
const typeFilter = filterType => {
  ...
  return variable => {
    ...
    const types = variable.detail.split('|').map(t => t.trim()).filter(Boolean);
    return types.includes(filterType);
  };
};

const writtenOnlyFilter = (writtenOnly, selectedElements) => {
  if (!writtenOnly) {
    return () => true;
  }
