...
export const EXTERNAL_SCOPE_ID = 'external-references';
...
export default function useGroupedVariables(sortedVariables, selectedElementIds) {
  return useMemo(() => {
    ...
    for (const variable of sortedVariables) {
      const scopeId = variable.scope?.id || EXTERNAL_SCOPE_ID;
    ...
    return Array.from(groups.values()).sort((a, b) => {
      ...
      // external references always group last
      const aIsExternal = a.scopeId === EXTERNAL_SCOPE_ID;
      const bIsExternal = b.scopeId === EXTERNAL_SCOPE_ID;

      if (aIsExternal !== bIsExternal) {
        return aIsExternal ? 1 : -1;
      }

      const aIsProcess = is(a.scope, 'bpmn:Process');
      const bIsProcess = is(b.scope, 'bpmn:Process');
