...
import { getValueNode } from './treeUtils';


const STRUCTURAL_CHARS = new Set([ '{', '}', '[', ']', ',', ':' ]);

const openMenuEffect = StateEffect.define();
...
function isPropertyName(node) {
  return (node.name === 'Name' || node.name === 'StringLiteral') &&
    node.parent?.name === 'Key' &&
    node.parent?.parent?.name === 'ContextEntry';
}

function isContainerNode(node) {
  return node.name === 'Context' || node.name === 'List';
}
...
export function interactiveControls(variableName, onMenuChange) {
